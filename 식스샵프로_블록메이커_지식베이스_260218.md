...
• 키워드: Luxury, Minimal, Artistic, Fashion
...
- 개발(블록 코드)
- QA 검증 및 자동 수정
을 Sixshop Block Maker GPT 수준으로 수행하도록 만드는 운영체계입니다.
...
# 설정 패널 설계 절대 원칙(강제 규칙)

## 0) 탭 3개 고정
- TAB: '콘텐츠' / '디자인' / '개발자'
- 콘텐츠 탭: 운영자용(비개발자) 설정만 둔다.
- 디자인/개발자 탭: developerMode true일 때만 하위 옵션을 보여준다.

## 1) 운영자 친화 설명 규칙(모든 setting 강제)
label:
- 기능이 한눈에 이해되는 명사형(예: “좌우 여백(모바일)”)

description: 반드시 아래 문장을 포함(줄바꿈 포함)
- “이 값은 OO을(를) 변경합니다.”
- “권장: XX.”
- “문제가 생기면 XX로 되돌리세요.”
- “엔터로 줄바꿈이 가능해요.”

## 2) Desktop/Mobile 2트랙 분리 규칙(필요하면 무조건)
강제 분리 대상:
- 제목/본문/버튼 텍스트(길이/줄바꿈/가독성)
- 정렬/레이아웃(모바일 1열, 배치 변경)
- 간격/패딩(모바일 타이트)

권장 네이밍:
- content.title.desktop / content.title.mobile
- content.body.desktop / content.body.mobile

## 3) “엔터 줄바꿈” 처리 규칙
- 줄바꿈 필요 텍스트는 TEXTAREA 사용
- template에서 white-space: pre-line; 처리
- 결론: 납품형(client_delivery)은 제목/설명도 TEXTAREA가 안전

## 4) 그룹핑 규칙(IA 고정)
설정 패널 IA는 아래로 고정한다(TITLE, DESCRIPTION으로 강제):
1) 콘텐츠(Content)
2) 레이아웃(Layout)
3) 디자인(Design)
4) 효과/특수 기능(Effects & Advanced)

## 5) 컬러 규칙(필수)
- 컬러는 RADIO로 ‘단색/그라데이션’ 선택
- 단색 선택 시: color picker 1개
- 그라데이션 선택 시: 시작색/끝색 color picker + 각도 range
- 디자인 탭에 배치하되 developerMode true일 때만 노출

## 6) 개발자 모드 노출 규칙(필수)
- developerMode CHECKBOX는 항상 존재
- 기본값 false
- 디자인/개발자 탭의 모든 옵션은 developerMode true일 때만 노출
